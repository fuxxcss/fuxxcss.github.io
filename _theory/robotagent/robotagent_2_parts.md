...
自注意力机制
    ...
    核心中的核心：如前所述，它让模型能够权衡序列中所有元素的重要性。

    多头注意力：这是对自注意力的一个增强。模型不是只做一次自注意力计算，而是并行地执行多次（多个“头”），每个“头”可以学习关注不同方面的信息（例如，一个头关注语法结构，另一个头关注指代关系）。最后将所有头的输出合并起来。

前馈神经网络

    这是什么：这就是标准的全连接神经网络，通常包含一个或两个隐藏层和激活函数（如 ReLU 或 GELU）。

    在 Transformer 中的角色：在自注意力层之后，会对每个位置的表示独立地应用一个前馈网络。它的作用是进行特征变换和非线性映射，将注意力机制提取的信息进行加工和升维/降维。

    与 CNN/RNN 的关系：FFN 本身是任何深度学习模型的基础组件，CNN 和 RNN 的末端也经常会接上全连接层进行分类或回归。
...
构建从初始状态到期望目标状态的潜在路径的过程 S0 → {a1, a2, . . . , an} → Sg ，其中 S0 是开始状态，A = {a1, a2, . . . , an}表示行动空间，Sg 是规划的目标状态。
...
从可用的选项中选择最佳的行动，at = D（ai），其中 ai 来自规划的当前行动序列 A = {a1, a2, . . . , an}
...
M 奖励 = (S, A, r, γ),其中 S 表示状态空间， A  表示行动空间，r(s, a) 指定奖励函数，γ ∈ [0, 1] 是折现因子。
...
agent感知环境生成观察结果 ot，ot = P(st, Mt−1),其中 Mt−1 引导选择性注意和过滤。
