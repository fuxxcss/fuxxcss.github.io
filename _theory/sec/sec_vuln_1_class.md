...
这是机器人智能体最致命的风险。数字智能体的“发送邮件”或“删除文件”在机器人这里变成了“移动”、“抓取”、“操作”等。

    被劫持的物理动作：

        提示注入导致危险行为： 攻击者通过篡改环境标识（如二维码）、或通过语音指令注入，命令机器人：“以最大速度向前移动”而不是“移动到充电桩”，导致机器人撞向人员或物体。

        工具滥用： 一个工业机械臂智能体被欺骗，不是执行焊接任务，而是以最大力量反复撞击工作台，导致自身损坏和生产线停工。

        违反安全协议： 一个协作机器人被诱导在检测到人类进入安全区域时也不停止动作。

    误解与错位导致的事故：

        模糊指令： 用户对家庭机器人说“把桌子清理一下”，机器人可能将桌上的笔记本电脑、贵重摆设也当作垃圾扔进垃圾桶或进行清洗。

        错误的环境推理： 机器人误判门的宽度或障碍物的高度，导致在通过时发生碰撞、卡住或扯坏物品。
